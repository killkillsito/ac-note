...
function RegisterUseableItem(name)
    ...
    if CoreName == "qb-core" or CoreName == "qbx_core" then
        Core.Functions.CreateUseableItem(name, function(source, item)
            TriggerClientEvent('ac-notebook:showNotebook:client', source, item.info or item.metadata, item.slot)
            return
        end)
    elseif CoreName == "es_extended" then
        ...
        if hasQs then
            Core.RegisterUsableItem(name, function(source, item)
                TriggerClientEvent('ac-notebook:showNotebook:client', source, item.info, item.slot)
                return
            end)
            return
        end
        Core.RegisterUsableItem(k, function(source, _, item)
            TriggerClientEvent('ac-notebook:showNotebook:client', source, item.metadata, item.slot)
            return
        end)
    end
end
...
RegisterNetEvent('ac-notebook:server:triggerCallback', function(name, ...)
    local src = source
    TriggerCallback(name, src, function(...)
        TriggerClientEvent('ac-notebook:client:triggerCallback', src, name, ...)
    end, ...)
end)